#!/usr/bin/env bash
# Ralph loop for scion lifecycle implementation.
#
# Usage:
#   ./notes/2026-03-01-scion-lifecycle/ralph.sh          # run until done (max 30 iterations)
#   ./notes/2026-03-01-scion-lifecycle/ralph.sh 10        # run up to 10 iterations
#
# The loop runs claude --print with the prompt, which reads the plan, picks the
# next incomplete task, implements it, verifies it, critiques it, and commits.
# The loop exits when all tasks are marked [x] or max iterations are reached.
#
# Environment variables:
#   RALPH_MODEL  — model to use (default: sonnet)
