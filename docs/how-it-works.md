<!-- Generated by Graft | model=bedrock-claude-v4.5-sonnet-us | sig=0cf439eae6c0feefc0d1a54cd16f53be58fe60d5ddb5d76a184d5a6c293fddc2 | rev=1c8d15b | 2025-11-08 -->
...
Source changes are detected by running `git diff` on all files listed in the `deps:` frontmatter section. The detection logic in `pack_prompt.py` (lines ~88-100) uses:

```bash
git diff HEAD~1 HEAD -- <source_files>
```

If any dependencies have changed between commits, `sources_changed` is set to `True` and a unified diff is generated showing exactly what changed in the source material.

### Prompt Changes

Prompt changes are detected by comparing the current prompt body (everything after the frontmatter) with the version from the previous commit. The logic (lines ~154-169) uses:

```bash
git show HEAD~1:<prompt_file>
```
...
The previous draft is always loaded from git HEAD (lines ~145), not from the filesystem. This ensures reproducible builds based on committed state, even if the working directory contains uncommitted changes to the output file. If the file doesn't exist in git (first generation), the previous draft is an empty string.
...
Files with the following extensions are treated as attachments (detected by `is_attachment_file()` function in `pack_prompt.py:68-71`):
- **PDFs**: `.pdf`
...
2. **Separates** text dependencies from attachment dependencies (lines ~134-135)
3. **Tracks changes** via git file hash using `check_file_changed_in_git()` (lines ~73-86)
4. **Outputs metadata** to `build/<name>.attachments.json` (lines ~229-231)

The `render_llm.sh` script then:

1. **Reads** the attachments list from the JSON file (lines ~38-52)
2. **Sanitizes filenames** by replacing periods with hyphens to satisfy Bedrock's document name validation
3. **Copies attachments** to the current directory with sanitized names
4. **Passes each attachment** to the LLM using the `-a` flag: `llm -m <model> -a file-pdf`
5. **Lets Claude** read and interpret the document natively

### Change Detection for Attachments

When an attachment changes:
- Git detects the binary file hash changed (checked via `git diff --quiet HEAD~1 HEAD -- <file>`)
...
The action selection logic (defined in `pack_prompt.py:172-183`) determines which directive to pass to Claude based on what changed:
