<!-- Generated by Graft | model=bedrock-claude-v4.5-sonnet-us | sig=d837eba1c2250ba6f50f83b80db5437f3df1c7556518ccee59ed14da030a86a0 | rev=1c8d15b | 2025-11-08 -->
...
```
🔧 Generating dvc.yaml from prompt files...
📋 Pipeline has 3 stage(s) to run
🚀 Running DVC pipeline...
Running stage 'exec_summary'
📝 Rendering: exec-summary
🤖 Using model: bedrock-claude-v4.5-sonnet-us
✅ Completed: exec-summary
```
...
```
💡 AWS credentials expired. To fix:
   - Run: aws sso login
...
```yaml
deps:
  - docs/strategy/messaging.md      # ✅ Directly referenced
  # - docs/legal/disclaimers.md    # ❌ Not actually used
```
...
If too many documents depend on a frequently-changing file, consider:

1. **Split the dependency** - Extract stable content into a separate file
2. **Make dependencies more specific** - Only depend on files you actually use
3. **Reorganize content** - Group frequently-changing content separately

**Example refactoring:**

❌ **Before:**
```
foundations.md (changes daily)
  ↓
exec-summary.md, recommendations.md, roadmap.md (all regenerate daily)
```

✅ **After:**
```
foundations-core.md (stable)
  ↓
exec-summary.md, recommendations.md (stable)

foundations-updates.md (changes daily)
  ↓
roadmap.md (regenerates when needed)
```
...
```bash
# Make a typical change
echo "Updated date" >> docs/strategy/foundations-updates.md

# Check what will regenerate
bin/graft status
```

Should show only the documents that truly depend on the changed file.
...
### 2. Review Recent Pipeline Output

```bash
# Full pipeline output with errors
bin/graft rebuild 2>&1 | tee graft-debug.log
```
...
# Check packed prompts
cat build/*.promptpack.txt
```
...
echo "Hello" | llm -m bedrock-claude-v4.5-sonnet-us
```
...
1. **Symptom** - What you expected vs. what happened
2. **Environment** - Docker version, OS, AWS region
3. **Reproduction steps** - Minimal commands to reproduce
4. **Logs** - Output from `graft-debug.log`
5. **Configuration** - Relevant parts of your `.prompt.md` files (sanitized)

**Sanitize sensitive data:**
- Remove AWS account numbers
- Remove internal project names
- Remove proprietary content

**Example issue title format:**
```
[Bug] Pipeline fails with "ExpiredToken" despite valid AWS credentials
```

File issues at: https://github.com/yourorg/graft/issues
