<!-- Generated by Graft | model=bedrock-claude-v4.5-sonnet-us | sig=e7a7409d7e03e043454df6037bb462ad0a79eb5f28c04ed0dcd14aba122c697c | rev=1c8d15b | 2025-11-08 -->
...
Graft supports PDFs and images as dependencies through native attachment handling. Files with extensions `.pdf`, `.png`, `.jpg`, `.jpeg`, `.gif`, or `.webp` are passed directly to Claude as attachments rather than being diffed as text.

When an attachment changes, Git detects the binary file hash change, and Claude receives the complete updated file to interpret with its native document understanding. This preserves formatting, tables, and layout without requiring extraction pipelines. Attachments are tracked in `build/<name>.attachments.json` and passed to the LLM using the `-a` flag.

Benefits of this approach:
- **No extraction needed** - Claude reads PDFs directly
- **Formatting preserved** - Tables, figures, and layout remain intact
- **Git-native versioning** - PDFs tracked like any other source
...
deps:
  ...
  - sources/research-paper.pdf    # PDF attachment
  - images/diagram.png            # Image attachment
---
```
...
- PDFs and images are passed as attachments, not diffed
